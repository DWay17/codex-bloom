...
public class BloomHash implements IBloomHash {
	...
	public String createBase64Result(String firstName, String lastName, char gender, Date bithdate) {
		LocalDate localDate = convertDate2localDate(bithdate);
		return createBase64Result(firstName, lastName, gender, localDate);
	}

	@Override
	public String createBase64Result(String firstName, String lastName, char gender, int birthYear, int birthMonth,
			int birthDay) {
		LocalDate localDate = convertInts2localDate(birthYear, birthMonth, birthDay);
		return createBase64Result(firstName, lastName, gender, localDate);
	}
	...
	public String createBase64Result(String firstName, String lastName, char gender, LocalDate bithdate) {
		...
		firstName = normalize(ChangeCase.UPPER, firstName, vocFirstName, removeFirstName, transFirstName);
		firstName = trim(firstName);
		firstName = padding(firstName);
		bs.or(randomHash(firstName, firstNameTable));

		lastName = normalize(ChangeCase.UPPER, lastName, vocLastName, removeLastName, transLastName);
		lastName = trim(lastName);
		lastName = padding(lastName);
		bs.or(randomHash(lastName, lastNameTable));

		String genderS = normalize(ChangeCase.LOWER, Character.toString(gender).toLowerCase(), vocGender, removeGender,
				transGender);
	...
	public String createBase64Result(String firstName, String lastName, char gender, String birthday) {
		LocalDate localDate = convertString2localDate(birthday);
		return createBase64Result(firstName, lastName, gender, localDate);
	}

	BitSetFixedSize balanceBloomFilter(BitSetFixedSize bloomFilter) {
		/* Balanced Bloom filters can be constructed by concatenating a Bloom filter with length l with a negated copy
		 * of the same Bloom filter. */
	...
	LocalDate convertDate2localDate(Date bithdate) {
		return LocalDate.ofInstant(bithdate.toInstant(), ZoneId.systemDefault());
	}

	LocalDate convertInts2localDate(int birthYear, int birthMonth, int birthDay) {
		return LocalDateTime.of(birthYear, birthMonth, birthDay, 0, 0, 0).toLocalDate();
	}

	LocalDate convertString2localDate(String birthday) {
		int birthYear;
		int birthMonth;
		int birthDay;
...
//	}
	...
	String normalize(ChangeCase changeCase, String s, String voc, Set<String> removeSet,
	...
	private void logTable(String name, LinkedHashMap<String, BitSetFixedSize> table) {
		...
		for (Iterator<Entry<String, BitSetFixedSize>> iterator = entries.iterator(); iterator.hasNext() && i < 3; i++) {
			Entry<String, BitSetFixedSize> entry = iterator.next();
